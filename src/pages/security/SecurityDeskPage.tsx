  ...
  return (
    <DashboardLayout>
      <div className="space-y-5">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by name, Crawford number, or destination..."
              ...
              <div key={req.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {req.checkout_time && (
                      <p className="text-xs text-slate-400 mt-1">Checked out: {new Date(req.checkout_time).toLocaleString()}</p>
                    )}
                    {req.checkin_time && (
                      <p className="text-xs text-slate-400 mt-1">Checked in: {new Date(req.checkin_time).toLocaleString()}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {req.status === 'APPROVED_FINAL' && (
                      <button
                        onClick={() => { openRequest(req); setAction('checkout'); }}
                    ...
                    {req.status === 'CHECKED_OUT' && (
                      <button
                        onClick={() => { openRequest(req); setAction('checkin'); }}
      ...
      <Modal
        ...
        onClose={() => { setSelected(null); setDocUrl(null); }}
        title="Exeat Details"
        size="lg"
      >
        {selected && (
      ...
      <Modal
        ...
        title={action === 'checkout' ? 'Confirm Check-Out' : 'Confirm Check-In'}
        size="sm"
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={() => setAction(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 font-medium transition-colors text-sm">
              Cancel
            </button>
            <button
