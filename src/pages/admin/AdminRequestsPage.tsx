      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {isToday
              ? "All exeat requests submitted today — view only"
              : role === 'hall_admin'
        ...
        {isToday && (
          <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm text-blue-700 font-medium flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center">
            <p className="text-slate-500">
              {isToday ? 'No requests submitted today' : 'No requests found'}
            </p>
          </div>
        ) : (
          <>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => openRequest(req)}
                              className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-blue-600 transition-colors" title="View">
                              <Eye className="w-4 h-4" />
                            </button>
                            {canApprove(req) && (
                              <button onClick={() => { openRequest(req); setActionModal('approve'); setComment(''); }}
                                className="p-1.5 rounded-lg text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-colors" title="Approve">
                                <CheckCircle className="w-4 h-4" />
                              </button>
                            )}
                            {canReject(req) && (
                              <button onClick={() => { openRequest(req); setActionModal('reject'); setComment(''); }}
                                className="p-1.5 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors" title="Reject">
      ...
      <Modal isOpen={!!selectedRequest && !!actionModal} onClose={() => { setActionModal(null); setComment(''); }}
        ...
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            {actionModal === 'approve' ? 'Add an optional comment before approving this request.' : 'Provide a reason for rejecting this request (required).'}
          </p>
          <div>
