...
router.post('/', protect, requireRole('student'), upload.single('document'), async (req, res) => {
  try {
    ...
    if (!destination || !reason_description || !reason_category || !departure_date || !return_date) {
    ...
    const request = await ExeatRequest.create({
      student_id: studentId, destination, reason_description, reason_category,
      departure_date, return_date, total_days: totalDays, academic_session: session,
      supporting_document_path: req.file.filename, supporting_document_name: req.file.originalname,
...
router.put('/:id', protect, requireRole('student'), upload.single('document'), async (req, res) => {
  try {
    ...
    if (totalDays > 5) return res.status(400).json({ message: 'Exeat duration cannot exceed 5 days.' });
    const overlaps = await ExeatRequest.findOne({
      student_id: req.user._id, _id: { $ne: existing._id },
      status: { $in: ['PENDING_HALL_ADMIN', 'APPROVED_BY_HALL_ADMIN', 'APPROVED_FINAL', 'CHECKED_OUT'] },
      departure_date: { $lte: return_date }, return_date: { $gte: departure_date },
    });
    if (overlaps) return res.status(400).json({ message: 'You already have an active exeat within the selected dates.' });
    existing.destination = destination; existing.reason_description = reason_description;
    existing.reason_category = reason_category; existing.departure_date = departure_date;
    existing.return_date = return_date; existing.total_days = totalDays;
    if (req.file) { existing.supporting_document_path = req.file.filename; existing.supporting_document_name = req.file.originalname; }
    await existing.save();
...
router.get('/my', protect, requireRole('student'), async (req, res) => {
  try {
    const requests = await ExeatRequest.find({ student_id: req.user._id })
      .populate('student_id', 'full_name crawford_number role').sort({ created_at: -1 });
...
router.get('/my/stats', protect, requireRole('student'), async (req, res) => {
  try {
    ...
    res.json({
      total: all.length,
      pending: all.filter(r => r.status === 'PENDING_HALL_ADMIN').length,
      approved: all.filter(r => ['APPROVED_BY_HALL_ADMIN', 'APPROVED_FINAL', 'CHECKED_OUT', 'CHECKED_IN'].includes(r.status)).length,
